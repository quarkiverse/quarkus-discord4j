...
If your Quarkus app uses the link:{quarkus-guides}/micrometer[Micrometer] extension, you can enable metrics collection for your bot exposed at the `/q/metrics` endpoint by setting the configuration property `quarkus.discord4j.metrics-enabled` to true.
...
include::includes/quarkus-discord4j-commands.adoc[]
